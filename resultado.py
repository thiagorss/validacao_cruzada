...
class Resultado():
    ...
    def mat_confusao(self) -> np.ndarray:
        ...
        #A matriz de confusão terá o tamanho como o máximo entre os valores dResultae self.y e self.predict_y
        self._mat_confusao = {}
    ...
    @property
    def macro_f1(self):
        return  sum(self.f1_por_classe.values())/len(self.mat_confusao)
...
class Fold():
    ...
    @staticmethod
    def gerar_k_folds(df_dados,val_k:int,col_classe:str,
                      num_repeticoes:int=1,seed:int=1,
                      num_folds_validacao:int=0,
                      num_repeticoes_validacao:int=1) -> List["Fold"]:
        """
        ...
        for num_repeticao in range(num_repeticoes):
            # para cada repetição, deve-se haver uma seed diferente: seed + num_repeticao
            df_dados_rand = df_dados.sample(frac=1, random_state=(seed+num_repeticao))

            #Impressão dos ids dos dados (exibe o print para testes)
            #print("Dados: "+str(df_dados_rand.index.values))
            for num_fold in range(val_k):
                # num_instances_per_partition e num_fold para deliminar o inicio e fim do teste
                ...
                #por meio do df_dados_rand, obtém os dados de avaliação (teste ou validação)
                df_to_predict = df_dados_rand[ini_fold_to_predict:fim_fold_to_predict]
                #print(df_to_predict)

                #Cria o treino por meio dos dados originais (df_dados_rand),
                #removendo os dados que serão avaliados  (df_to_predict)
                df_treino = df_dados_rand.drop(df_to_predict.index, axis=0)
                #print(df_treino)

                #Cria o fold (objeto da classe Fold) para adicioná-lo no vetor
